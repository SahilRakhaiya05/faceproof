...
| Face comparison | “Are these two face crops similar enough to review as the same person?” | OpenCV SFace, calibrated threshold, human approval | Legal identity, authorship, or a universally correct match |
| Web discovery | “Where has this or a visually/same-person-related image appeared in an indexed public page?” | FaceCheck for same-person search; Google Lens via SerpApi for exact/cropped/reposted imagery | Complete web coverage, platform endorsement, or that the result is true |
...
|---|---|---|---|
| [FaceCheck API](https://facecheck.id/en/Face-Search/API) | Vendor-operated internet face index; upload a query image and poll a search job | Best available adapter here for finding a **different photograph of the same person** | Paid, proprietary index and scoring; coverage/freshness are not independently established. Vendor docs say test mode scans only 100,000 faces and results are not meaningful, so it cannot substantiate the rubric demo. Treat the vendor as a biometric processor and check its terms before use. |
| [Google Lens through SerpApi](https://serpapi.com/google-lens-api) | SerpApi fetches Google Lens visual/exact-match results | Strong fallback for the same, cropped, resized, or reposted image; `no_cache=true` bypasses SerpApi's one-hour cache | Reverse-image/visual search, **not face identification**. It depends on two vendors and changing upstream markup/indexes. Disabling SerpApi's cache does not guarantee upstream freshness. |
...
**Recommended routing:** try FaceCheck production mode for same-person discovery, use Lens for exact/near-duplicate discovery or as a fallback, then download the candidate image and locally rerank it with SFace. Preserve the raw provider response and retrieval time. A result URL or provider score is a candidate lead, not a signed statement from the social platform.
...
| Open-web face index | **Buy/API: FaceCheck, consented production account** | Building a useful, lawful web-scale biometric index is outside hackathon scope and creates the largest legal/ethical risk. If unavailable, label the run degraded rather than substituting a hardcoded URL. |
| Exact/near-duplicate web search | **Buy/API: SerpApi Lens; optionally Cloud Vision Web Detection** | Genuine, repeatable query step with raw response capture. It complements rather than replaces face search. |
