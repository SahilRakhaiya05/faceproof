...
**Recommended routing:** try FaceCheck production mode for same-person discovery, use Lens for exact/near-duplicate discovery or as a fallback, then download the candidate image and locally rerank it with SFace. Preserve a sanitized parsed response, the exact HTTP-body digest, request/search ID, and retrieval time. A result URL or provider score is a candidate lead, not a signed statement from the social platform.
...
| Exact/near-duplicate web search | **Buy/API: SerpApi Lens; optionally Cloud Vision Web Detection** | Genuine, repeatable query step with a sanitized response and exact-body digest. It complements rather than replaces face search. |
